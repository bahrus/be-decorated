...
## Soapbox, repetitive, thinking out loud ramble
...
I strongly believe we want to be able to enhance elements during template instantiation, using the same class definitions, and we should not require some fixed attribute name associated with a giant string of configurations being cloned every time to do this, in my opinion.  Yes, we will need a [registry of names](https://github.com/bahrus/trans-render#non-verbal-spells-via-make-transforms) to describe our enhancements, but I maintain it shouldn't require the presence of an attribute adorning the element.

There may be more than one angle for how we want to enhance existing elements.  The angle be-decorated takes is squarely on adding "behaviors".  Every dream I've had since 1994, when it comes to enhancing HTML has been associated with adding things that are nicely described as behaviors.  But I'm not sure that perfectly describes *all* types of enhancements people might want to make.  Things that are look and feel related (that may define which CSS theme to pull in for example) don't really fit that.  And there may be other categories.  Which is why I come back to "decorators", to match what we call those things in JavaScript.  And I really like the phrase "adorned element."  And decorators seems like it could aptly describe CSS themes also, no?
